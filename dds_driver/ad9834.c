...
static unsigned int ad9834_calc_freqreg(unsigned long mclk, unsigned long fout)
{
	...
	printk("mclk=%ld fout=%ld",mclk,fout);
	do_div(freqreg, mclk);
	return freqreg;
}
...
static int ad9834_probe(struct spi_device *spi)
{
	...
	mclk=clk_get(&spi->dev,"mclk");

	printk("mclk  rate =%ld",clk_get_rate(mclk));

	st->mclk = clk_get_rate(mclk);
	st->spi = spi;
	...
	ret = ad9834_write_frequency(st, AD9834_REG_FREQ0, 1000);//pdata->freq0);
	if (ret)
		goto error_disable_reg;

	ret = ad9834_write_frequency(st, AD9834_REG_FREQ1, 2000);//pdata->freq1);
	if (ret)
		goto error_disable_reg;

	ret = ad9834_write_phase(st, AD9834_REG_PHASE0, 0);//pdata->phase0);
	if (ret)
		goto error_disable_reg;

	ret = ad9834_write_phase(st, AD9834_REG_PHASE1, 0);//pdata->phase1);
	if (ret)
		goto error_disable_reg;
